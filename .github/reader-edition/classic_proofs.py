...
ODE_LINKS=[('complex-analysis.html', 'Polygonal complex analysis'), ('fuchs.html', 'Fuchs’s theorem'), ('painleve.html', 'Painlevé’s classification')]
LINKS += ODE_LINKS
...
def install(site,revision,euler_audit,cauchy_audit,source_file=None):
    ...
    for name,title in [('leibniz','The Leibniz series'),('basel','The Basel problem'),('euler','Euler’s sine-product proof'),('fuchs','Fuchs’s theorem'),('painleve','Painlevé’s classification'),('complex-analysis','Polygonal complex analysis')]:
    ...
    for asset in ['cauchy-example.css','cauchy-example.js']:
    ...
    report=dict(documentationRevision=revision,cartwrightProofRevision=audit['sourceCommit'],
      ...
      checks=dict(cartwrightAlreadyProved=True,cartwrightThreeCheckedRoutesPreserved=True,leibnizComparisonPreserved=True,baselNativeAudited=True,mathlibSourcePinned=True,eulerProofAudited=True,allEvenZetaValuesAudited=True,polygonalCauchyAudited=True),
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--site',required=True,type=Path);p.add_argument('--revision',required=True);p.add_argument('--mathlib-source',type=Path);p.add_argument('--euler-audit',required=True,type=Path);p.add_argument('--cauchy-audit',required=True,type=Path)
    a=p.parse_args();install(a.site,a.revision,a.euler_audit,a.cauchy_audit,a.mathlib_source)

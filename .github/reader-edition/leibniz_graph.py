...
def validate(model):
    ...
    assert len(nodes) == len(model['nodes']) == 21
    ...
    for n in model['nodes']:
        ...
        for ref in n['refs']:
            revisions = [model['mathlibRevision']] if 'leanprover-community' in ref['url'] else [model['caRevision'], model.get('transmutationRevision', '__REVISION__')]
            assert any('/blob/'+revision+'/' in ref['url'] for revision in revisions)
    return nodes

def overview(model):
    """Static SVG remains usable without JavaScript and links to the explorable map."""
    ns=validate(model);esc=html.escape
    positions={n['id']:(18+n['col']*318,30+n['row']*133,288+(n['span']-1)*318,104) for n in model['nodes']}
    parts=['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 833" role="img" aria-labelledby="title desc"><title id="title">Four routes to the Leibniz formula</title><desc id="desc">Finite inequalities, polynomial FTC and geometric transmutation lead to geometric pi. Mathlib uses real and complex analysis and Abel’s endpoint theorem.</desc><defs><marker id="a" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto"><path d="M0 0L10 5L0 10z" fill="#82958a"/></marker></defs>']
    for r in model['routes']:
        parts.append(f'<text x="{24+({0:0,1:1,2:3,3:2}[r["id"]])*318}" y="16" font-family="sans-serif" font-size="10" fill="#587365">{esc(r["name"].upper())}</text>')
    ...
    for n in model['nodes']:
        ...
        color,stroke=('#f1f4ed','#adbaa9') if len(n['routes'])>1 else [('#eff5ef','#779784'),('#eef4f7','#89a8ba'),('#f4eff8','#b2a0c3'),('#eff5ef','#779784')][n['routes'][0]]
...
def install(site, revision):
    ...
    model=json.loads((SOURCE/'model.json').read_text().replace('__REVISION__',revision));model['transmutationRevision']=revision;validate(model)
    ...
    assert old
    figure=START+'''<figure style="margin:24px 0"><a href="leibniz-proofs.html" aria-label="Explore the Leibniz proof routes"><img src="reading/leibniz-overview.svg" alt="Dependency graph: computational, FTC and transmutation routes share rational foundations and geometric pi; Mathlib uses completed reals, the arctangent series and Abel’s theorem." width="1280" height="833" style="display:block;width:100%;height:auto"></a><figcaption style="font-size:14px;margin-top:12px"><a href="leibniz-proofs.html">Explore all four proof routes →</a> Use the cosine-primitive-style viewer to select mathematical bundles, follow routes, and open formal sources.</figcaption></figure>'''+END
    ...
    replacements={'__OUTLINE__':''.join(outline),'__CA__':model['caRevision'],'__ML__':model['mathlibRevision'],'__CA_SHORT__':model['caRevision'][:8],'__ML_SHORT__':model['mathlibRevision'][:8],'__SCOPE__':html.escape(model['edgeSemantics'])+' The new transmutation theorem is audited separately; the Mathlib outline remains based on its pinned source.'}
    ...
    report=dict(documentationRevision=revision,caProofRevision=model['caRevision'],mathlibProofRevision=model['mathlibRevision'],page=PAGE,bundleCount=len(model['nodes']),edgeCount=len(model['edges']),mathematicalOutline=True,extractedDeclarationGraph=False,newLeanProofsClaimed=True,transmutationProofRevision=revision,cosinePrimitiveViewerPattern=True,previousChapterSha256=hashlib.sha256(original.encode()).hexdigest(),chapterSha256=digest(page),artifactHashes={p:digest(site/p) for p in artifacts},protectedArtifactHashes=protected,checks=dict(priorChapterOutsideDiagramPreserved=True,existingProofMapsUnchanged=True,pinnedSources=True,acyclicDependencies=True,noCrossFoundationEdges=True))
    previous_report.write_text(json.dumps(report,indent=2)+'\n')
    print('PASS: 21 mathematical bundles, 4 routes; prior chapter prose and proof maps preserved')
    return report

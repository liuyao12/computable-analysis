...
MACHIN_TERMS=r'''\begin{aligned}\frac\pi4={}&4\left(\frac15-\frac1{3\cdot5^3}+\frac1{5\cdot5^5}-\cdots\right)\\
&-\left(\frac1{239}-\frac1{3\cdot239^3}+\frac1{5\cdot239^5}-\cdots\right).
\end{aligned}'''

def machin_card():
    body='<article class="pi-formula-card" id="pi-machin"><h3>Machin</h3>'+formula(MACHIN_MAIN)
    body+='<p>Two small rational arguments make the alternating series converge rapidly.</p><details class="pi-formula-details"><summary>Series pattern and error bound</summary>'+formula(MACHIN_TERMS)
    body+='<p>Use N terms of each alternating series:</p>'+formula(r'T_N(q)=\sum_{k=0}^{N-1}\frac{(-1)^kq^{2k+1}}{2k+1},\qquad p_N=16T_N(1/5)-4T_N(1/239).')
    body+=formula(r'|\pi-p_N|\le\frac{16}{(2N+1)5^{2N+1}}+\frac4{(2N+1)239^{2N+1}}.')
    body+='<p>The arctangent addition law is finite rational algebra: doubling 1/5 gives 5/12, doubling again gives 120/119, and subtracting the angle of 1/239 gives tangent 1. The angle range selects π/4, not another branch.</p>'
    body+='<p>This also gives the third logarithm computation in the neighboring card; its integral or series is not evaluated by reading an existing π value.</p><a href="'+SOURCE+'" target="_blank" rel="noopener">Formula and arctangent addition identities →</a></details><a href="#pi-logarithm">The same formula as a logarithm computation →</a></article>'
    return parse(body).article
...
def install(site,revision):
    ...
    print('PASS: Newton factor pattern; Machin series; three rational Log(i) computations; original proofs and worked illustrations unchanged')

...
existing proof anchors and audit artifacts. The browser checks cover all ten
...
The arctangent page is the function-theory showcase. It compares geometric
arctangent with literal Taylor polynomials, with a persistent interactive
function plot, partial-sum sequence, and off-axis singularity view. The
publication pass requires `--arctan-audit`, generated by
`lake env lean scripts/check_arctan_taylor.lean`. The checked classification
currently quantifies over rational inputs; the larger classical statement
and the represented-input extension boundary are explicitly distinguished.

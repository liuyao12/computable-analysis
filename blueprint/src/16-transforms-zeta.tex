...
\section{Checked zeta computation for real exponents greater than one}
The Lean implementation now constructs a valid real output for every represented
input $s>1$: a finite input box certifies this strict inequality. It selects a
rational chart $1+1/(q+1)\le s\le m+2$, $q>0$, and uses the canonical adaptive
application of an interval-regular function.

For rational exponents in this chart, define
\[
 c_0(s)=1,\qquad c_{k+1}(s)=\frac{k+2-s}{k+1}c_k(s),\qquad
 D_k(N)=\sum_{n<N}\frac{(1-1/(n+1))^k}{(n+1)^2}.
\]
The finite rectangle $R(s,K,N)=\sum_{k<K}c_k(s)D_k(N)$ is exactly a finite
Dirichlet sum whose powers are evaluated by binomial polynomials.
The coefficients are the unique formal solution of $(1-z)f'=(2-s)f$,
$f(0)=1$. This uniqueness statement concerns coefficients, not an analytic
derivative theorem or the separate log--exponential power implementation.

With $J=m+1$ and $B=(m+3)^J$, the checked estimates include
\[
 (k+1)D_k(N)\le2,\qquad
 |R(s,K+L,N)-R(s,K,N)|\le2(q+1)|c_K(s)|\quad(K\ge J),
\]
\[
 |R(s,K,N+L)-R(s,K,N)|\le\frac{KB}{N}\quad(N>0),\qquad
 |c_{J2^j}(s)|\le B\left(\frac{2q+2}{2q+3}\right)^j.
\]
These prove termination of the cutoff search and validity of the stabilized
boxes. Finite polynomial Lipschitz bounds provide the interval regularity
needed for arbitrary represented real inputs.

\begin{theorem}[Checked real-domain zeta construction]
\label{thm:zeta-real-computation}
\lean{ComputableAnalysis.ZetaReal.zeta_valid, ComputableAnalysis.ZetaReal.zeta_equiv, ComputableAnalysis.ZetaReal.dirichlet_convergence, ComputableAnalysis.ZetaReal.zeta_integer_equiv}
\leanok
The construction returns a valid represented real for every input strictly
above one, is independent of the input implementation and selected chart,
and extends the existing integer zeta evaluator. The finite Dirichlet sums
converge effectively in rational output enclosures.
\end{theorem}
\begin{proof}
The two finite tail estimates give a Cauchy estimate between arbitrary
rectangles; prefix intersection gives valid nested output boxes. Lipschitz
bounds and adaptive input refinement give the real-input extension. Common
sufficiently large rectangles prove chart independence. At integer exponents
the binomial series terminates and the rectangles become the earlier finite
Dirichlet sums, proving compatibility, in particular at $s=3$.
\end{proof}

Agreement with the integer evaluator does not prove Euler's even-value formula
\[
 \zeta(2n)=\frac{(2\pi)^{2n}|B_{2n}|}{2(2n)!},\qquad n\ge1.
\]
The general formula remains open, but its first case is now proved.

\begin{theorem}[Basel and the conditional Euler proof of prime infinitude]
\label{thm:basel-primes}
\lean{ComputableAnalysis.Basel.eulerBasel, ComputableAnalysis.Basel.real_zeta_two_equiv_piSquaredOverSix, ComputableAnalysis.Basel.prime_unbounded_of_piSquare_irrational}
\leanok
The reciprocal-square computation and the real-domain zeta computation at $2$
are equivalent to $\pi^2/6$, for geometric $\pi$. If $\pi^2$ is irrational,
there is a prime greater than every given natural bound.
\end{theorem}
\begin{proof}
\leanok
Put $B_N=\sum_{k<N}(-1)^k/(2k+1)$, $H_N=\sum_{k<N}1/(2k+1)$, and
$M_N=\sum_{k<N}1/(2k+1)^2$. Partial fractions, finite diagonal rearrangements,
and alternating-sum bounds prove
\[
 |2B_N^2-M_N|\le 2H_N/N.
\]
The odd/even split and the reciprocal-square tail estimate imply
\[
 \left|\sum_{k=1}^{2m^2}k^{-2}-\frac83 B_{2m^2}^{\,2}\right|\le16/m.
\]
This gives equivalence of the interval computations; the existing geometric
Leibniz theorem completes Basel. No completed real line is involved.

For a finite list $P$ of distinct primes, let $S_P(N)$ sum reciprocal squares
of the positive integers up to $N$ divisible by none of the primes in $P$, and
$c_P=\prod_{p\in P}(1-p^{-2})$. Finite sieving proves
\[
 S_{p::P}(N)=S_P(N)-p^{-2}S_P(\lfloor N/p\rfloor),\qquad
 \left|S_P(N)-c_P\sum_{k=1}^N k^{-2}\right|\le\frac{K_P}{N+1},
\]
where $K_{\varnothing}=0$ and $K_{p::P}=(p+1)K_P+2p$.
An exhaustive list would give $S_P(N)=1$ and hence rational $\zeta(2)=1/c_P$,
contradicting Basel and the stated irrationality hypothesis. The proof uses
finite prime factorization, not the Euclidean infinitude theorem.
\end{proof}

Irrationality of $\pi^2$ remains a separate proof obligation. The finite Basel
argument adapts \href{https://doi.org/10.4169/college.math.j.44.3.171}{Benko and
Molokach's rearrangement proof}; the formal error bounds supply the limiting
steps.

The reference evaluator is not optimized for speed near $s=1$. Its identification
with $E(-sL(n))$, parameter derivatives, general Euler products, and continuation remain
additional proof obligations. The following continuation discussion is a
mathematical construction plan, not a claim that those Lean certificates are
already available.

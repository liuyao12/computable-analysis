...
The organizing principle is to construct the objects and prove their laws.
A general theorem may begin with supplied constructions and proved evidence
of their mathematical roles, and then derive a new conclusion.  Producing
those objects under more general sufficient conditions is a separate theorem.
A useful conditional law need not wait for the weakest possible existence
hypotheses.  In a concrete application, however, we must construct its inputs
and establish the required evidence.  Naming a derivative, integral, or
solution does not provide that evidence, and storing a desired conclusion in
a record is not a proof of it.

Methods for choosing enclosures, tails, subdivisions, and local models belong
in the formalization skill.  Their particular finite computations and proofs
belong in Lean.  Reusable proof records are optional organization, rather than
universal definitions of every integral, series, or solution.  The distinction
between a construction, a conditional law, and a comparison should remain
visible in every theorem's scope.  A mathematical narrative in a later chapter
does not by itself assert that the corresponding Lean construction is complete.
...
\input{06-ftc-precision.tex}
\input{04-infinite-series.tex}
\input{05-exponential-logarithm.tex}
\input{07-algebra-fta.tex}
\input{12-local-models.tex}
\input{13-complex-paths.tex}
\input{14-improper-parameters.tex}
\input{10-fourier.tex}
\input{11-impulses.tex}
\input{09-differential-equations.tex}
\input{15-special-function-equations.tex}
\input{16-transforms-zeta.tex}
\input{10-algebraic-ode.tex}

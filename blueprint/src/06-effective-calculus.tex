...
rational refinement.  This theorem constructs a valid raw integral; by itself
it does not identify that integral with a primitive endpoint difference.
\end{theorem}
...
endpoint raw real.  The derivative-bound version is the current reusable
calculus interface: local derivative bounds on rational partition cells are
summed and compared with endpoint differences.  This is the formal proof-route
object that can support arctangent-integral pi equivalence targets.
\end{theorem}
...
The intended Lean proof should manufacture the effective FTC certificate from
the one-sided derivative inequalities below.
\end{theorem}

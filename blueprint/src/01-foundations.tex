...
\[
  x\sim \sigma\ldot x.
\]
Thus, to prove \(x\sim y\), it is enough to find schedules
\(\sigma,\tau\) and prove
\[
...
distinction is clear, we simply call elements of \(\R\) real numbers.
\end{definition}
...
The embedding \(\Q\hookrightarrow\R\) sends \(q\) to the class of
\[
  [q,q]_{n\in\N}.
\]
It preserves \(0\), \(1\), addition, negation, multiplication, inverses of
nonzero rational numbers, and order.
\end{theorem}

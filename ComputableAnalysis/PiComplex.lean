...
/-- Doubling the accelerated sector-area endpoint is a certified pi raw
equivalent to the preferred circle-area representation.  This is the direct
whole-angle counterpart of `sectorAreaAngleOne_equiv_halfPi`; it leaves the
scoreboard unchanged because it reuses the rectangle-integral capability. -/
theorem sectorAreaPiRaw_equiv_piCircleArea :
    SectorAreaRotation.piRaw.Equiv circleArea.raw := by
  exact RealRaw.equiv_trans
    SectorAreaRotation.piRaw_valid
    fourArctanGeomOneValid
    circleArea.valid
    SectorAreaRotation.piRaw_equiv_fourArctanGeomOne
...
/-- The stabilized factorial rotation of the project-level \(\pi/2\) handle
also agrees with the rotation obtained directly from the accelerated
sector-area endpoint.  Thus the Euler route has one common factorial
rotation after both raw-angle transports; identifying it with the geometric
endpoint \(i\) remains the separate vector-uniqueness step. -/
theorem halfPiRotation_equiv_sectorAreaRotation :
    halfPiRotation.Equiv SectorAreaRotation.rotation := by
  exact ComplexRaw.equiv_trans
    halfPiRotation_valid
    GeometricPiRotation.rotation_valid
    SectorAreaRotation.rotation_valid
    halfPiRotation_equiv_geometricRotation
    (ComplexRaw.equiv_symm
      SectorAreaRotation.rotation_equiv_geometricRotation)
...
/-- The project-level complex input \(i\pi/2\) also agrees with the input
formed directly from the accelerated sector-area endpoint.  This is the
complex-input counterpart of
`halfPiRotation_equiv_sectorAreaRotation`; neither theorem asserts the
remaining exponential/geometric endpoint equality. -/
theorem imaginaryHalf_equiv_sectorAreaRotationImaginaryHalf :
    imaginaryHalf.Equiv SectorAreaRotation.imaginaryHalf := by
  exact ComplexRaw.equiv_trans
    imaginaryHalf_valid
    GeometricPiRotation.imaginaryHalf_valid
    SectorAreaRotation.imaginaryHalf_valid
    imaginaryHalf_equiv_geometricImaginaryHalf
    (ComplexRaw.equiv_symm
      SectorAreaRotation.imaginaryHalf_equiv_geometricImaginaryHalf)

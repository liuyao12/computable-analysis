...
/-- The square is monotone on every nonnegative rational subinterval.  This
shared range estimate belongs to the retained square FTC construction, not to
one of the removed higher-degree regressions. -/
theorem square_range_on_nonneg {a b x : Rat}
  ...
  exact {
    ...
    contains_values := fun n x hx => by
      unfold squareIntegralBound squareIntegralDerivativeRaw RealFunRaw.exact
        QInterval.ContainsInterval
      have hrange := square_range_on_nonneg C.lower_mem C.ordered hx.1 hx.2

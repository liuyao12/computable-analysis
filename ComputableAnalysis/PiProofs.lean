...
/-- The finite endpoints of the positive arctangent evaluator are precisely
the odd and even finite primitives of its kernel polynomial over `[0,y]`.
This is a finite rational identity: no limiting integral is used. -/
theorem endpoints_eq_kernelPartialIntegralBetween (y : Rat) (n : Nat) :
  ...
  | zero =>
      ...
      · unfold lo
        rw [state_succ, state_zero]
        simp [Taylor.ArctanKernel.kernelPartialIntegralBetween,
          Taylor.ArctanKernel.kernelTermIntegralBetween]
        grind [Rat.sub_eq_add_neg]
  | succ n ih =>
      have hloAsStep :
          hi y n - powState y n / (4 * (n : Rat) + 3) =
            Taylor.ArctanKernel.kernelPartialIntegralBetween 0 y (2 * n + 1) := by
        rw [← ih.2]
        unfold lo
        rw [state_succ]
      ...
      have hhiSucc :
        ...
        rw [show (state y n).2.2 = powState y n by rfl]
        rw [hloAsStep, powState_eq]
...
theorem hi_eq_kernelPartialIntegralBetween_even (y : Rat) (n : Nat) :
    hi y n = Taylor.ArctanKernel.kernelPartialIntegralBetween 0 y (2 * n) :=
  (endpoints_eq_kernelPartialIntegralBetween y n).1

theorem lo_eq_kernelPartialIntegralBetween_odd_succ (y : Rat) (n : Nat) :
    lo y (n + 1) =
...
def lowerKernelPartialAtStage (y : Rat) : Nat -> Rat
  | 0 => 0
  | n + 1 => Taylor.ArctanKernel.kernelPartialIntegralBetween 0 y (2 * n + 1)
...
theorem lo_eq_lowerKernelPartialAtStage (y : Rat) (n : Nat) :
  ...
  cases n with
  | zero =>
      simp [lo, state_zero, lowerKernelPartialAtStage]
  | succ n =>
      simp [lowerKernelPartialAtStage,
        lo_eq_kernelPartialIntegralBetween_odd_succ y n]

theorem hi_eq_upperKernelPartialAtStage (y : Rat) (n : Nat) :
    hi y n = upperKernelPartialAtStage y n := by
  simp [upperKernelPartialAtStage,
    hi_eq_kernelPartialIntegralBetween_even y n]

theorem positiveRaw_compute_eq_kernelPartialIntegralInterval (y : Rat) (n : Nat) :
    (positiveRaw y).compute n =
      { lo := lowerKernelPartialAtStage y n,
        hi := upperKernelPartialAtStage y n } := by
  simp [positiveRaw, lo_eq_lowerKernelPartialAtStage,
    hi_eq_upperKernelPartialAtStage]

theorem arctan_compute_eq_kernelPartialIntegralInterval
    {x : Rat} (hx0 : 0 <= x) (n : Nat) :
    (arctan x).compute n =
      { lo := lowerKernelPartialAtStage x n,
        hi := upperKernelPartialAtStage x n } := by
  rw [arctan_compute_nonneg x hx0 n]
  have hqabs : qabs x = x := by
    unfold qabs
    have hnot : Not (x < 0) := by grind
    simp [hnot]
  rw [hqabs]
...
/-- Refining the finite midpoint partition preserves the exact finite
primitive of every kernel polynomial on `[0,x]`. -/
...
/-- On the dyadic midpoint mesh of `[0,x]`, the finite kernel polynomial
differs from its exact rational primitive by at most
`m(m+1)x² / 2^n`. -/
theorem arctanAreaLoopState_kernelPartialRightRectangle_error_bound
  ...
  have hsum := kernelPartialRightRectangleSum_error_bound hunit
...
private theorem leibniz_bridge_even_error_le_radius (n : Nat) :
    (2 * (n : Rat)) * ((2 * n + 1 : Nat) : Rat) *
        (1 / (((2 ^ leibnizRectangleBridgeMeshStage n : Nat) : Rat))) <=
      1 / (((2 ^ n : Nat) : Rat)) := by
  convert leibniz_bridge_error_le_dyadic n using 1 <;> push_cast <;> ring
...
private theorem scheduledRectangle_lower_le_leibniz_upper_plus_radius
  ...
  have hlower := LeibnizRectangleBridge
    .integralLowerSum_le_kernelPartialRightRectangleSum_even n hunit
  have herr := LeibnizRectangleBridge
    .arctanAreaLoopState_one_kernelPartialRightRectangle_error_bound
      (2 * n) stage
...
private theorem leibniz_lower_le_scheduledRectangle_upper_plus_radius
  ...
  | succ n =>
      ...
      have hunit : ArctanGeometry.UnitIntervals intervals := by
        ...
        exact ArctanGeometry.arctanAreaLoopState_intervals_unit
          (x := (1 : Rat)) (by native_decide) (by native_decide) stage
      have hupper := LeibnizRectangleBridge
        .kernelPartialRightRectangleSum_odd_le_integralUpperSum n hunit
      have herr := LeibnizRectangleBridge
        .arctanAreaLoopState_one_kernelPartialRightRectangle_error_bound
          (2 * n + 1) stage
...
/-- Restricting a finite kernel Riemann estimate from `[0,1]` to a
nonnegative subinterval `[0,x]` can only improve its mesh factor. -/
private theorem kernel_mesh_error_le_unit_mesh
    {x : Rat} (hx0 : 0 <= x) (hx1 : x <= 1) (m stage : Nat) :
    (m : Rat) * ((m + 1 : Nat) : Rat) *
        ((x * x) / (((2 ^ stage : Nat) : Rat))) <=
      (m : Rat) * ((m + 1 : Nat) : Rat) *
        (1 / (((2 ^ stage : Nat) : Rat)) ) := by
  have hsq : x * x <= 1 := by
    calc
      ...
      _ = x := by rw [Rat.mul_one]
      _ <= 1 := hx1
  have hdenpos : 0 < (((2 ^ stage : Nat) : Rat)) :=
    (Rat.natCast_pos).2 (Nat.pow_pos (by omega : 0 < 2))
  have hinv : 0 <= 1 / (((2 ^ stage : Nat) : Rat)) :=
    Rat.le_of_lt (Rat.div_pos (by native_decide) hdenpos)
  have hmesh :
      (x * x) / (((2 ^ stage : Nat) : Rat)) <=
        1 / (((2 ^ stage : Nat) : Rat)) := by
    rw [Rat.div_def, Rat.div_def]
    simpa using Rat.mul_le_mul_of_nonneg_right hsq hinv
  have hcoeff : 0 <= (m : Rat) * ((m + 1 : Nat) : Rat) :=
    Rat.mul_nonneg Rat.natCast_nonneg Rat.natCast_nonneg
  exact Rat.mul_le_mul_of_nonneg_left hmesh hcoeff
...
private theorem scheduledRectangle_lower_le_arctan_upper_plus_radius
  ...
  have hlower := LeibnizRectangleBridge
    .integralLowerSum_le_kernelPartialRightRectangleSum_even n hunit
  have herr := LeibnizRectangleBridge
    .arctanAreaLoopState_kernelPartialRightRectangle_error_bound
      x hx0 hx1 (2 * n) stage
  have hmesh := kernel_mesh_error_le_unit_mesh hx0 hx1 (2 * n) stage
  have hradius := leibniz_bridge_even_error_le_radius n
  change ArctanGeometry.integralLowerSum intervals <=
    ArctanValidity.upperKernelPartialAtStage x n +
      1 / (((2 ^ n : Nat) : Rat))
  ...
  calc
    ArctanGeometry.integralLowerSum intervals <=
        LeibnizRectangleBridge.kernelPartialRightRectangleSum (2 * n)
          intervals := hlower
...
private theorem arctan_lower_le_scheduledRectangle_upper_plus_radius
    (x : Rat) (hx0 : 0 <= x) (hx1 : x <= 1) (n : Nat) :
    ArctanValidity.lowerKernelPartialAtStage x n <=
      (ArctanGeometry.arctanIntegralRectangleRaw x).compute
        (leibnizRectangleBridgeMeshStage n).hi +
        1 / (((2 ^ n : Nat) : Rat)) := by
  cases n with
  ...
  | succ n =>
      ...
      have herr := LeibnizRectangleBridge
        .arctanAreaLoopState_kernelPartialRightRectangle_error_bound
          x hx0 hx1 (2 * n + 1) stage
      have hmesh := kernel_mesh_error_le_unit_mesh hx0 hx1 (2 * n + 1) stage
      have hradius := leibniz_bridge_odd_error_le_radius n
      change ArctanValidity.lowerKernelPartialAtStage x (n + 1) <=
        ArctanGeometry.integralUpperSum intervals +
          1 / (((2 ^ (n + 1) : Nat) : Rat))
      have hleft :
          Taylor.ArctanKernel.kernelPartialIntegralBetween 0 x (2 * n + 1) <=
          LeibnizRectangleBridge.kernelPartialRightRectangleSum (2 * n + 1)
            intervals + ((2 * n + 1 : Nat) : Rat) *
              ((2 * n + 2 : Nat) : Rat) *
                ((x * x) / (((2 ^ stage : Nat) : Rat))) := by
...
/-- A scheduled, finitely padded rectangle representative for the arctangent
series at a nonnegative unit input.  The padding is a vanishing dyadic zero
interval, cancelled below after finite overlap has been established. -/
def arctanRectangleBridgePaddedRaw (x : Rat) : RealRaw :=
  (RealRaw.schedule leibnizRectangleBridgeMeshSchedule
    (ArctanGeometry.arctanIntegralRectangleRaw x) -
      leibnizBridgeDyadicZero) + leibnizBridgeDyadicZero
...
private theorem arctan_equiv_arctanRectangleBridgePaddedRaw
  ...
  rw [ArctanValidity.arctan_compute_eq_kernelPartialIntegralInterval hx0 n]
  change QInterval.Overlaps
    ...
    { lo := (ArctanGeometry.arctanIntegralRectangleRaw x).compute
      ...
      hi := (ArctanGeometry.arctanIntegralRectangleRaw x).compute
          ...
          (1 / (((2 ^ n : Nat) : Rat))) }
  unfold QInterval.Overlaps
  have hlower := arctan_lower_le_scheduledRectangle_upper_plus_radius
    x hx0 hx1 n
  have hupper := scheduledRectangle_lower_le_arctan_upper_plus_radius
    x hx0 hx1 n
  constructor <;> grind [Rat.sub_eq_add_neg]
...
/-- The positive arctangent power-series evaluator agrees with its geometric
rectangle construction on every rational input in `[0,1]`, by finite Taylor
polynomials and rational midpoint meshes alone. -/
theorem arctanEqualsRectangleRaw_finiteRiemannBridge
    (x : Rat) (hx0 : 0 <= x) (hx1 : x <= 1) :
    (arctan x).Equiv (ArctanGeometry.arctanIntegralRectangleRaw x) := by
  have hseries : (arctan x).Valid :=
  ...
  have htoScheduled : (arctan x).Equiv
      (RealRaw.schedule leibnizRectangleBridgeMeshSchedule
        (ArctanGeometry.arctanIntegralRectangleRaw x)) :=
    RealRaw.equiv_trans hseries
      (arctanRectangleBridgePaddedRaw_valid x hx0 hx1) hscheduled
      (arctan_equiv_arctanRectangleBridgePaddedRaw x hx0 hx1)
      (arctanRectangleBridgePaddedRaw_equiv_scheduledRectangle x hx0 hx1)
  have hschedule :
      (RealRaw.schedule leibnizRectangleBridgeMeshSchedule
        (ArctanGeometry.arctanIntegralRectangleRaw x)).Equiv
        (ArctanGeometry.arctanIntegralRectangleRaw x) :=
    RealRaw.equiv_symm
      (RealRaw.schedule_equiv (ArctanGeometry.arctanIntegralRectangleRaw x)
        (ArctanGeometry.arctanIntegralRectangleRaw_valid hx0 hx1)
        leibnizRectangleBridgeMeshSchedule)
  exact RealRaw.equiv_trans hseries hscheduled
    (ArctanGeometry.arctanIntegralRectangleRaw_valid hx0 hx1)
    htoScheduled hschedule

/-- Kernel-comparison form of the finite Riemann bridge, ready for the
Machin series inputs. -/
theorem powerSeriesEqualsRectangleKernelAt_finiteRiemannBridge
    (x : Rat) (hx0 : 0 <= x) (hx1 : x <= 1) :
    PowerSeriesEqualsRectangleKernelAt x hx0 hx1 := by
  have hseries : (arctan x).Valid :=
    arctan_valid_at arctanValid (arctanDomain_of_nonnegativeUnit hx0 hx1)
  have hrectangle : (ArctanGeometry.arctanIntegralRectangleRaw x).Valid :=
    ArctanGeometry.arctanIntegralRectangleRaw_valid hx0 hx1
  have hkernel :
      (Taylor.ArctanComparison.kernelIntegralRaw x
        (rectangleKernelIntegralAtNonnegativeUnit x hx0 hx1)).Valid :=
    Taylor.ArctanComparison.kernelIntegralRaw_valid x
      (rectangleKernelIntegralAtNonnegativeUnit x hx0 hx1)
  exact RealRaw.equiv_trans hseries hrectangle hkernel
    (arctanEqualsRectangleRaw_finiteRiemannBridge x hx0 hx1)
    (rectangleKernelIntegralRaw_equiv_rectangleRaw_nonnegativeUnit x hx0 hx1)

/-- Geometric-arctangent form of the finite Riemann bridge. -/
theorem arctan_equiv_arctanGeom_finiteRiemannBridge
    (x : Rat) (hx0 : 0 <= x) (hx1 : x <= 1) :
  ...
  have hgeom : (ArctanGeometry.arctanGeom x).Valid :=
    ArctanGeometry.arctanGeom_valid_on_unit hx0 hx1
  exact RealRaw.equiv_trans hseries hrectangle hgeom
    (arctanEqualsRectangleRaw_finiteRiemannBridge x hx0 hx1)
    (ArctanGeometry.arctanIntegralRectangleRaw_equiv_arctanGeom hx0)
...
/-- The direct Machin series evaluator reaches the certified circle-area pi
value using only its two small arctangent inputs.  Both series/geometric
agreements come from the finite rational Riemann bridge; the Machin identity
itself remains the bounded rational tangent-chart theorem. -/

...
theorem leftStieltjesSum_scale_right (c : Rat) (f g : Nat -> Rat) (n : Nat) :
    leftStieltjesSum f (fun i => c * g i) n =
      c * leftStieltjesSum f g n := by
  induction n with
  | zero =>
      change 0 = c * 0
...
theorem rightStieltjesSum_scale_right (c : Rat) (f g : Nat -> Rat) (n : Nat) :
    rightStieltjesSum f (fun i => c * g i) n =
      c * rightStieltjesSum f g n := by
  induction n with
  | zero =>
      change 0 = c * 0
...
theorem rightStieltjesSum_one_right (f : Nat -> Rat) (n : Nat) :
    rightStieltjesSum f (fun _ => (1 : Rat)) n = f n - f 0 := by
  induction n with
  | zero =>
      grind [rightStieltjesSum]
  | succ n ih =>
      rw [rightStieltjesSum, ih]
      grind [Rat.sub_eq_add_neg, Rat.add_assoc, Rat.add_comm]

theorem rightStieltjesSum_const_right (f : Nat -> Rat) (c : Rat) (n : Nat) :
    rightStieltjesSum f (fun _ => c) n = c * (f n - f 0) := by
  induction n with
  | zero =>
      grind [rightStieltjesSum]
  | succ n ih =>
...
theorem leftStieltjesSum_const_path (f : Nat -> Rat) (c : Rat) (n : Nat) :
    leftStieltjesSum f (fun _ => c) n = 0 := by
  induction n with
  | zero =>
      rfl
  | succ n ih =>
...
theorem leftStieltjesSum_affine_path (f g : Nat -> Rat)
    (s t : Rat) (n : Nat) :
    leftStieltjesSum f (fun i => s * g i + t) n =
      s * leftStieltjesSum f g n := by
  calc
    leftStieltjesSum f (fun i => s * g i + t) n =
        leftStieltjesSum f (fun i => s * g i + (fun _ => t) i) n := by
          rfl
    _ = leftStieltjesSum f (fun i => s * g i) n +
        leftStieltjesSum f (fun _ => t) n := by
          exact leftStieltjesSum_add_right f
...
/-! Nonnegative rational scaling also commutes with the finite rectangle
fold.  Together with `riemannLeftInterval_add`, this is the finite algebraic
core of linearity for the integral candidate. -/

theorem riemannLeftInterval_scale_of_nonneg
    {r : Rat} (hr : 0 <= r) (f : RealFunRaw) (a b : Rat)
    (subdivisions prec : Nat) :
...
/-! Negative scaling reverses interval endpoints.  The ordered-interval
hypothesis makes the mesh nonnegative, so this is still a literal finite
rectangle identity rather than a limiting argument. -/

theorem riemannLeftInterval_scale_of_neg
...
/-- The schedule-aware form permits the two interval evaluators to use
independent stages.  The valid point computation supplies the cross-stage
overlap needed to join their two containment certificates. -/
theorem IntervalRegularOn.evalIntervals_overlap_of_common_point_at
    {F : FunctionOnInterval}
    (left right : IntervalRegularOn F)
    (I : QInterval) (hI : subintervalOf I F.lower F.upper)
    (x : Rat) (hx : inDomainInterval F.lower F.upper x)
    (hlo : I.lo <= x) (hhi : x <= I.hi)
    (leftStage rightStage : Nat) :
    QInterval.Overlaps
      (left.evalInterval I hI leftStage)
      (right.evalInterval I hI rightStage) := by
  have hleft := left.contains_point_values I hI x hx leftStage hlo hhi
  have hright := right.contains_point_values I hI x hx rightStage hlo hhi
  ...
  change (right.evalInterval I hI rightStage).lo <=
      (F.compute x hx rightStage).lo /\
    (F.compute x hx rightStage).hi <=
      (right.evalInterval I hI rightStage).hi at hright
...
/-- Scheduled interval regularity also gives rational epsilon-delta
continuity.  The only additional bookkeeping is that both point boxes are
read at the certificate's explicit `evalPrecision n`; no limit object or
completed real line is introduced. -/
theorem ScheduledIntervalRegularOn.epsilonDeltaContinuous
    {F : FunctionOnInterval} (h : ScheduledIntervalRegularOn F) :
    EpsilonDeltaContinuousOn F := by
  intro eps
  let n : Nat := eps.val.den
  ...
  have hyhi : y <= I.hi := by
    dsimp [I]
    grind
  let p : Nat := h.evalPrecision n
  have hcontainsX := h.contains_point_values I hI x hx n hxlo hxhi
  have hcontainsY := h.contains_point_values I hI y hy n hylo hyhi
  have hXwidth_nonneg := (F.valid_on x (F.defined_on x hx)).1 p
  have hYwidth_nonneg := (F.valid_on y (F.defined_on y hy)).1 p
  have hXordered : (F.compute x hx p).lo <= (F.compute x hx p).hi := by
    change 0 <= (F.compute x hx p).hi - (F.compute x hx p).lo at hXwidth_nonneg
    grind [Rat.sub_eq_add_neg]
  have hYordered : (F.compute y hy p).lo <= (F.compute y hy p).hi := by
    change 0 <= (F.compute y hy p).hi - (F.compute y hy p).lo at hYwidth_nonneg
    grind [Rat.sub_eq_add_neg]
  have hwidthX : (F.compute x hx p).width <= eps.val :=
    Rat.le_trans (QInterval.width_le_of_contains hcontainsX) hYwidth
  have hwidthY : (F.compute y hy p).width <= eps.val :=
    Rat.le_trans (QInterval.width_le_of_contains hcontainsY) hYwidth
  unfold QInterval.NearAt
  constructor
  · change (F.compute x hx p).lo <= (F.compute y hy p).hi + eps.val
    unfold QInterval.ContainsInterval at hcontainsX hcontainsY
    unfold QInterval.width at hYwidth
    grind [Rat.sub_eq_add_neg]
  constructor
  · change (F.compute y hy p).lo <= (F.compute x hx p).hi + eps.val
    unfold QInterval.ContainsInterval at hcontainsX hcontainsY
    unfold QInterval.width at hYwidth
    grind [Rat.sub_eq_add_neg]
  exact ⟨hwidthX, hwidthY⟩
...
theorem applyRealRaw_contained_in_candidate_expand
    (F : ContinuousFunctionOnInterval) (x : RealRaw) (hx : x.Valid)
    (hsource : forall n,
      subintervalOf (x.compute n) F.function.lower F.function.upper)
    (n : Nat) :
    (QInterval.expand ((F.applyCandidate x hx hsource).compute n)
      (1 / (((n + 1 : Nat) : Rat)))).ContainsInterval
      ((F.applyRealRaw x hx hsource).compute n) := by
  exact RealRaw.prefixStabilize_contained_in_current_expand
    (F.applyCandidate x hx hsource) applyRadius n

theorem applyRealRaw_contains_candidate
    (F : ContinuousFunctionOnInterval) (x : RealRaw) (hx : x.Valid)
    (hsource : forall n,
      subintervalOf (x.compute n) F.function.lower F.function.upper)
    (n : Nat) :
    ((F.applyRealRaw x hx hsource).compute n).ContainsInterval
      ((F.applyCandidate x hx hsource).compute n) := by
...
/-! The endpoint range is a genuine Darboux bracket at the project's weak
interval-order level: monotonicity makes it overlap every certified point
computation in the cell.  Full interval containment would require a stronger
endpoint-order contract and is intentionally not claimed here. -/
theorem nondecreasingDarbouxRange_overlaps_point_value
    (F : FunctionOnInterval) (hF : NondecreasingOnInterval F)
    (P : RationalPartition F.lower F.upper)
    (k : Nat) (hk : k < P.pieces) (x : Rat)
    (hx : inDomainInterval F.lower F.upper x)
  ...
  have hupper : inDomainInterval F.lower F.upper C.upper :=
    And.intro (Rat.le_trans C.lower_mem C.ordered) C.upper_mem
  have hleft := hF C.lower x hlower hx hlo prec
  have hright := hF x C.upper hx hupper hhi prec
...
theorem intervalRegularDarbouxSchedule_widths_shrink_of_budget
    {F : FunctionOnInterval} (hregular : IntervalRegularOn F)
  ...
  intro n hn
  exact Rat.le_trans
    (intervalRegularDarbouxStage_width_le_of_uniform_input_budget F hregular
...
def IntervalRegularDarbouxSchedule.ofAutomaticPieces
    ...
    (nested : forall n m, n <= m ->
      (intervalRegularDarbouxScheduleCompute F hinterval hregular
        ...
        evalPrecision
        (fun n => intervalRegularAutomaticPieces_pos hregular lengthBound
          evalPrecision n) n).lo <=
      (intervalRegularDarbouxScheduleCompute F hinterval hregular
        (fun n => intervalRegularAutomaticPieces hregular lengthBound
          evalPrecision n)
        evalPrecision
        (fun n => intervalRegularAutomaticPieces_pos hregular lengthBound
          evalPrecision n) m).lo /\
      (intervalRegularDarbouxScheduleCompute F hinterval hregular
        (fun n => intervalRegularAutomaticPieces hregular lengthBound
          evalPrecision n)
        evalPrecision
        (fun n => intervalRegularAutomaticPieces_pos hregular lengthBound
          evalPrecision n) m).hi <=
      (intervalRegularDarbouxScheduleCompute F hinterval hregular
        (fun n => intervalRegularAutomaticPieces hregular lengthBound
          evalPrecision n)
        evalPrecision
        (fun n => intervalRegularAutomaticPieces_pos hregular lengthBound
          evalPrecision n) n).hi)
    :
    IntervalRegularDarbouxSchedule F hregular hinterval where
  pieces := fun n => intervalRegularAutomaticPieces hregular lengthBound
    evalPrecision n
  evalPrecision := evalPrecision
  pieces_pos := fun n => intervalRegularAutomaticPieces_pos hregular
...
/-- Canonical forward application on an inverse branch.  Input precision is
selected adaptively, so validity follows from `x.Valid` without a same-stage
rate assumption. -/
def forwardRealRaw (I : InvertibleFunctionOnInterval) (x : RealRaw)
    (hx : x.Valid)
    (hsource : forall n,
      subintervalOf (x.compute n) I.function.lower I.function.upper) :
    RealRaw :=
  I.continuous.applyRealRaw x hx hsource

theorem forwardRealRaw_valid
    (I : InvertibleFunctionOnInterval) (x : RealRaw) (hx : x.Valid)
    (hsource : forall n,
      subintervalOf (x.compute n) I.function.lower I.function.upper) :
    (I.forwardRealRaw x hx hsource).Valid :=
  I.continuous.applyRealRaw_valid x hx hsource
...
private theorem not_strictly_separated_of_forward_equiv
    ...
    (hforward : (I.forwardRealRaw x hx hxsource).Equiv
      (I.forwardRealRaw y hy hysource)) :
  ...
  | nondecreasing =>
      ...
      have hpn : p <= n := by
        dsimp [n]
        omega
      have hdenn : eps.val.den <= n := by
        dsimp [n]
        omega
      have hsmall : 1 / (((n + 1 : Nat) : Rat)) <= eps.val := by
        exact Rat.le_trans (one_div_nat_antitone_succ hdenn)
          (one_div_den_succ_le_of_pos eps.property)
      let sx : Nat := I.continuous.inputStage x hx n
      let sy : Nat := I.continuous.inputStage y hy n
      have hnsx : n <= sx := I.continuous.le_inputStage x hx n
      have hnsy : n <= sy := I.continuous.le_inputStage y hy n
      have hNsx : N <= sx := Nat.le_trans hNn hnsx
      have hNsy : N <= sy := Nat.le_trans hNn hnsy
      let q : Rat := (x.compute sx).hi
      let r : Rat := (y.compute sy).lo
      have hxnested := hx.2.1 N sx hNsx
      have hynested := hy.2.1 N sy hNsy
      have hq_le_a : q <= a := by
      ...
      have hXcontains := I.continuous.regular.contains_point_values
        (x.compute sx) (hxsource sx) q hq n
        (by simpa [q] using (hxsource sx).2.1) (by simp [q])
      have hYcontains := I.continuous.regular.contains_point_values
        (y.compute sy) (hysource sy) r hr n
        (by simp [r]) (by simpa [r] using (hysource sy).2.1)
      have hqmono' :
      ...
      have hYcontains' :
          (I.function.compute r hr n).hi <=
            (I.continuous.regular.evalInterval
              (y.compute sy) (hysource sy) n).hi := by
        simpa [InvertibleFunctionOnInterval.function] using hYcontains.2
      have hXwidth' :
          (I.continuous.regular.evalInterval
            (x.compute sx) (hxsource sx) n).width <= eps.val := by
        exact Rat.le_trans
          (by simpa [ContinuousFunctionOnInterval.applyCandidate, sx] using
            I.continuous.applyCandidate_width x hx hxsource n)
          hsmall
      have hYwidth' :
          (I.continuous.regular.evalInterval
            (y.compute sy) (hysource sy) n).width <= eps.val := by
        exact Rat.le_trans
          (by simpa [ContinuousFunctionOnInterval.applyCandidate, sy] using
            I.continuous.applyCandidate_width y hy hysource n)
          hsmall
      have haUpper :
      ...
      have hXcandidateUpper :
          ((I.continuous.applyCandidate x hx hxsource).compute n).hi <=
            (I.function.compute a ha p).hi + eps.val := by
        change
          (I.continuous.regular.evalInterval
            (x.compute sx) (hxsource sx) n).hi <=
              (I.function.compute a ha p).hi + eps.val
        have hfromWidth :
            (I.continuous.regular.evalInterval
                (x.compute sx) (hxsource sx) n).hi <=
              (I.continuous.regular.evalInterval
                  (x.compute sx) (hxsource sx) n).lo + eps.val := by
          grind [QInterval.width, Rat.sub_eq_add_neg]
        exact Rat.le_trans hfromWidth
          (rat_add_le_add
            (Rat.le_trans hXcontains' (Rat.le_trans hqmono' haUpper))
            (Rat.le_refl : eps.val <= eps.val))
      have hYcandidateLower :
          (I.function.compute b hb p).lo - eps.val <=
            ((I.continuous.applyCandidate y hy hysource).compute n).lo := by
        change
          (I.function.compute b hb p).lo - eps.val <=
            (I.continuous.regular.evalInterval (y.compute sy) (hysource sy) n).lo
        have hfromWidth :
            (I.continuous.regular.evalInterval
                (y.compute sy) (hysource sy) n).hi - eps.val <=
              (I.continuous.regular.evalInterval
                (y.compute sy) (hysource sy) n).lo := by
          grind [QInterval.width, Rat.sub_eq_add_neg]
        have hpoint :
            (I.function.compute b hb p).lo <=
              (I.continuous.regular.evalInterval
                (y.compute sy) (hysource sy) n).hi :=
          Rat.le_trans hbLower (Rat.le_trans hrmono' hYcontains')
        grind [Rat.sub_eq_add_neg]
      have hXstabilized :=
        I.continuous.applyRealRaw_contained_in_candidate_expand
          x hx hxsource n
      have hYstabilized :=
        I.continuous.applyRealRaw_contained_in_candidate_expand
          y hy hysource n
      have hXupper :
          ((I.forwardRealRaw x hx hxsource).compute n).hi <=
            (I.function.compute a ha p).hi + eps.val + eps.val := by
        have hcontain := hXstabilized.2
        change ((I.forwardRealRaw x hx hxsource).compute n).hi <=
          ((I.continuous.applyCandidate x hx hxsource).compute n).hi +
            1 / (((n + 1 : Nat) : Rat)) at hcontain
        exact Rat.le_trans hcontain (by grind)
      have hYlower :
          (I.function.compute b hb p).lo - eps.val - eps.val <=
            ((I.forwardRealRaw y hy hysource).compute n).lo := by
        have hcontain := hYstabilized.1
        change
          ((I.continuous.applyCandidate y hy hysource).compute n).lo -
              1 / (((n + 1 : Nat) : Rat)) <=
            ((I.forwardRealRaw y hy hysource).compute n).lo at hcontain
        grind [Rat.sub_eq_add_neg]
      have hforwardStrict :
          ((I.forwardRealRaw x hx hxsource).compute n).hi <
            ((I.forwardRealRaw y hy hysource).compute n).lo := by
      ...
      have hover := (RealRaw.compareAt_overlap_iff
        (I.forwardRealRaw x hx hxsource)
        (I.forwardRealRaw y hy hysource) n n).1 (hforward n)
  ...
  | nonincreasing =>
      ...
      have hXcandidateLower :
          (I.function.compute a ha p).lo - eps.val <=
            ((I.continuous.applyCandidate x hx hxsource).compute n).lo := by
        change
          (I.function.compute a ha p).lo - eps.val <=
            (I.continuous.regular.evalInterval (x.compute sx) (hxsource sx) n).lo
        have hfromWidth :
            (I.continuous.regular.evalInterval
                (x.compute sx) (hxsource sx) n).hi - eps.val <=
              (I.continuous.regular.evalInterval
                (x.compute sx) (hxsource sx) n).lo := by
          grind [QInterval.width, Rat.sub_eq_add_neg]
        have hpoint :
            (I.function.compute a ha p).lo <=
              (I.continuous.regular.evalInterval
        ...
        grind [Rat.sub_eq_add_neg]
      have hYcandidateUpper :
          ((I.continuous.applyCandidate y hy hysource).compute n).hi <=
            (I.function.compute b hb p).hi + eps.val := by
        change
          (I.continuous.regular.evalInterval (y.compute sy) (hysource sy) n).hi <=
            (I.function.compute b hb p).hi + eps.val
        have hfromWidth :
            (I.continuous.regular.evalInterval
                (y.compute sy) (hysource sy) n).hi <=
              (I.continuous.regular.evalInterval
                  (y.compute sy) (hysource sy) n).lo + eps.val := by
          grind [QInterval.width, Rat.sub_eq_add_neg]
        exact Rat.le_trans hfromWidth
          (rat_add_le_add
      ...
      have hYstabilized :=
        I.continuous.applyRealRaw_contained_in_candidate_expand
          y hy hysource n
      have hXlower :
          (I.function.compute a ha p).lo - eps.val - eps.val <=
            ((I.forwardRealRaw x hx hxsource).compute n).lo := by
        have hcontain := hXstabilized.1
        change
          ((I.continuous.applyCandidate x hx hxsource).compute n).lo -
              1 / (((n + 1 : Nat) : Rat)) <=
            ((I.forwardRealRaw x hx hxsource).compute n).lo at hcontain
        grind [Rat.sub_eq_add_neg]
      have hYupper :
          ((I.forwardRealRaw y hy hysource).compute n).hi <=
            (I.function.compute b hb p).hi + eps.val + eps.val := by
        have hcontain := hYstabilized.2
        change ((I.forwardRealRaw y hy hysource).compute n).hi <=
          ((I.continuous.applyCandidate y hy hysource).compute n).hi +
            1 / (((n + 1 : Nat) : Rat)) at hcontain
        exact Rat.le_trans hcontain (by grind)
      have hforwardStrict :
...
their adaptive interval-forward evaluations are equivalent.  The proof uses only
  ...
  · by_cases h : (y.compute n).lo <= (x.compute n).hi
    ...
    · exfalso
      have hstrict : (x.compute n).hi < (y.compute n).lo := by
        simpa [Rat.not_le] using h
      exact (not_strictly_separated_of_forward_equiv I hresolves
        hx hy hxsource hysource hforward n) hstrict
...
theorem source_equiv_of_common_forward_target
    ...
    (hforwardX_target : (I.forwardRealRaw x hx hxsource).Equiv target)
    (hforwardY_target : (I.forwardRealRaw y hy hysource).Equiv target) :
    x.Equiv y := by
  have hforward :
      (I.forwardRealRaw x hx hxsource).Equiv
        (I.forwardRealRaw y hy hysource) :=
    RealRaw.equiv_trans
      (I.forwardRealRaw_valid x hx hxsource) htarget
      (I.forwardRealRaw_valid y hy hysource)
      hforwardX_target (RealRaw.equiv_symm hforwardY_target)
  exact I.source_equiv_of_forward_equiv hresolves
    hx hy hxsource hysource hforward
    ...
    forall y n,
      QInterval.Overlaps
        (I.continuous.regular.evalInterval
          ...
          n)
        (y.value.compute n)
  forward_equiv_target : forall y,
    (I.forwardRealRaw
      { compute := compute_preimage y }
      (valid_preimage y)
      (preimage_subinterval y)).Equiv y.value
...
theorem apply_forward_equiv_target {I : InvertibleFunctionOnInterval}
    (inv : InverseRaw I) (y : InRangeRaw I) :
    (I.forwardRealRaw (inv.apply y) (inv.apply_valid y)
      (inv.apply_stays_in_source y)).Equiv y.value := by
  simpa [apply] using inv.forward_equiv_target y
    ...
    forall n,
      QInterval.Overlaps
        (I.continuous.regular.evalInterval
          ...
          n)
        (y.value.compute n)
  forward_equiv_target :
    (I.forwardRealRaw
      { compute := compute_preimage }
      valid_preimage
      preimage_subinterval).Equiv y.value
...
adaptive forward image represents the same target.
...
theorem preimage_equiv_of_candidate
    ...
    (hcandidateSource : forall n,
      subintervalOf (candidate.compute n)
        I.function.lower I.function.upper)
    (hcandidateTarget : (I.forwardRealRaw candidate hcandidate
...
def inverseRawOfSearch {I : InvertibleFunctionOnInterval}
  ...
  preimage_subinterval := fun y => (search y).preimage_subinterval
  value_overlaps := fun y => (search y).value_overlaps
  forward_equiv_target := fun y => (search y).forward_equiv_target

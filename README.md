...
| Euler identity and complex logarithm (target) | $\exp(i\pi/2)=i$ and $\pi=-2i\log(i)$, tying complex exp/log to the rotation system and ODE uniqueness | `PiProofs.pi.halfPi` is a certified real parameter with every box in $[1,2]$ and width at most $2/(n+1)$; its `imaginaryHalf` embedding is the exact $(i/2)$ scalar action. The return $(-2i)(i\pi/2)=\pi$ and, conditionally on a certified `log(i)=i\pi/2` branch, the literal product $(-2i)\log(i)=\pi$ are checked. `ComplexRaw.cauchyStabilize` supplies the finite-prefix assembly for a future represented-input exponential; its factorial/Lipschitz containment proof and geometric identification remain planned. |
...
stabilized into a public FTC construction.  The supplied unit triangle
arctangent construction and logarithmic strip now split that product identity;
the remaining gate is a reusable monotone-piece theorem together with the
...
The actual bounded-sum evaluator is now normalized as
`coordinateTimesArctanForwardTwoStageStabilizedRaw`: its runtime intersects a
finite prefix of widened bounded-sum boxes, using only rational sums and the
explicit radius `4/(n+1)`. The rectangle evaluator is an anchor only in the
validity proof. Thus
...
general effective-FTC comparison certificate that connects arbitrary such
strip sums to displayed integrals.  The same bracket now produces a valid
...
because this particular mesh does not itself identify either strip sum with a
definite integral or with canonical `log 2`; the separate triangle construction
does supply the scoped unit-branch split later in this file.
...
identity, not a continuous Fubini axiom.  The supplied arctangent triangle
construction now packages this unit-branch prefix as an integral; extending
that identification to a general effective-FTC schema remains open.
...
`Logarithm.piTriangleLogSeries = 4 * arctan.integral.triangle +
2 * log.series(2)`.  Lean proves it equivalent first to four times the
product-FTC integral, then to `4 * arctan.geom(1)` and the preferred circle-
...
| `baselSeriesRaw` / `pi^2/6` | [definition](https://liuyao12.github.io/computable-analysis/ch-infinite-series.html#def:zeta-two-raw), [RHS valid](https://liuyao12.github.io/computable-analysis/ch-infinite-series.html#thm:basel-geometric-rhs-valid) | $\zeta(2)=\sum_{n\ge1}\frac1{n^2}=\pi^2/6$; area and direct-circumference RHSs agree | ✓ | ✗ |
